...
class ControllerPaymentOPCreditCard extends Controller {
	...
	public function index() {
 		...
 		if (isset($this->error['securecode'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
   			...
   			'href' => $this->url->link('payment/op_creditcard', 'token=' . $this->session->data['token'], 'SSL'),
   		);
				
		$data['action'] = $this->url->link('payment/op_creditcard', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['callback'] = HTTP_CATALOG . 'index.php?route=payment/op_creditcard/callback';
...
//存入session
      ...
      $this->session->data['op_creditcard_entitys'] = $data['op_creditcard_entitys'];
		...
		$this->response->setOutput($this->load->view('payment/op_creditcard.tpl', $data));
	}
